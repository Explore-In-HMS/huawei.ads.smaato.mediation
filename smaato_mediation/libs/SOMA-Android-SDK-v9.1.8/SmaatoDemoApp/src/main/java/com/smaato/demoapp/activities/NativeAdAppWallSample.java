...
public class NativeAdAppWallSample extends Activity implements OnClickListener,
		AdListenerInterface {
	NativeAd nativeAd;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_native_ad_app_wall_sample);
		reloadBanner = (Button) findViewById(R.id.load_ad );
		reloadBanner.setOnClickListener(NativeAdAppWallSample.this);
		nativeAd = new NativeAd(this);
		nativeAd.setAdListener(NativeAdAppWallSample.this);
		nativeRelativeLayout = (RelativeLayout) findViewById(R.id.nativeAdLayout);

		SharedPreferences prefs = NativeAdAppWallSample.this.getSharedPreferences(Constants.COM_SMAATO_DEMOAPP,
